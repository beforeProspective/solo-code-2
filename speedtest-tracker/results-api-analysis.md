...
- `created_at` / `updated_at`：由 `$table->timestamps()` 生成，底层是 `timestamp` 列，非空；Laravel Eloquent 在模型 `create()` / `save()` 时自动填充。
- `healthy`：由后续迁移以 `$table->boolean('healthy')->nullable()` 追加，**可空、无默认值**。

也就是说：一行结果在刚 `insert` 时就会带上 `created_at`，但 `healthy` 要等异步处理链路（例如 `ProcessCompletedSpeedtest`、`ProcessUnhealthySpeedtest` 等监听器或 Job）执行后才会被 `update` 为 `true`/`false`。在这之前数据库里的值是 `NULL`。
...
- `'created_at' => $this->created_at->toDateTimestring()` —— 总是有值；
...
- 控制器 `list()` 方法：使用 `QueryBuilder`，只通过 `allowedSorts` 暴露了 `ping`、`download`、`upload`、`created_at`、`updated_at`；**在未传 `?sort=` 参数时不会主动追加 `ORDER BY`**，结果顺序退化为数据库的“自然顺序”（MySQL/InnoDB 近似于主键升序，PostgreSQL/SQLite 则未定义）。
...
- MySQL InnoDB 默认会退回到**主键（`id`）升序**，因为 InnoDB 按聚簇索引存储；但这是实现细节，不是 SQL 标准；
...
3. **列表与 latest 不一致**：`list` 未显式排序时通常按 `id ASC`，第一行是“最早一条”；而 `latest()` 按时间取。即便都按时间排序，相同时刻下也可能不一致，前端在“最新结果”组件和“结果列表第一行”之间会看到错位；
...
三个方法都使用 `$request->user()->tokenCant('results:read')` 做相同的 token ability 校验，所以**权限上确实同一级别**。但：

- `list` 在无数据时返回空集合 `data: []`，语义是“查询成功、集合为空”；
- `latest` 使用 `firstOrFail()`，在无数据时抛出 `ModelNotFoundException`，Laravel 会渲染成 500（`ApiController::throw` 路径未被调用），与 `list` 的 200 + 空集完全不同；
- `show` 使用 `findOr` + 自抛 `NotFoundException`，在无数据时返回 404。

于是“列表能读”不代表 “latest 能返回一个可解析的 data”，它可能直接抛出 500。

### 3.2 排序规则不对称，语义不一致

- `list` 默认不显式排序（依赖数据库自然顺序，多半是 `id ASC`），所以“列表里的第一条”是**最早**的结果；
...
1. **错误语义不同**：`list` 空集 vs `latest` 500；
2. **排序语义不同**：`list` 自然顺序 vs `latest` 按时间倒序；
...
- 用 `firstOr(fn () => ...)` 或 `firstOrFail()` 的自定义异常统一为 404，避免 500。
